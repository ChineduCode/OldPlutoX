...
select.forEach(btn => btn.addEventListener('click', ()=>{
    ...
    selectActive[0].classList = selectActive[0].className.replace('select-active', '')
...
const solveForAmountToReceive = ()=>{
    if(payorReceive.innerText == 'You receive'){
        let amount = 0
        amount = parseFloat(amountInput.value) - (parseFloat(amountInput.value) * 2/100)
        if(amountInput.value == ''){
...
form.addEventListener('submit', (e) =>{
    ...
    paymentInput.forEach(input => {
        if(input.value === ''){
            ...
            }else if(input.classList.contains('exp-month') || input.classList.contains('exp-year')){
